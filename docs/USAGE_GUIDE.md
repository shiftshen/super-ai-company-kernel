...
如果本机 API / daemon 实际指向 `/Users/shift/openclaw/company-kernel`，则先切到真实运行目录：
...
```bash
bin/companyctl approval approve --approval-id <id> --by owner-shift --reason "确认执行"
```

拒绝审批：

```bash
bin/companyctl approval deny --approval-id <id> --by owner-shift --reason "证据不足"
```
